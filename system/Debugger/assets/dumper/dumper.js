(function() {
	...
	Debugger.Dumper = Debugger.Dumper || {};

	Debugger.Dumper.init = function(repository) {
		if (repository) {
			[].forEach.call(document.querySelectorAll('.debugger-dump[data-debugger-dump]'), function(el) {
				try {
					el.appendChild(build(JSON.parse(el.getAttribute('data-debugger-dump')), repository, el.classList.contains('debugger-collapsed')));
					el.classList.remove('debugger-collapsed');
					el.removeAttribute('data-debugger-dump');
				} catch (e) {
		...
		document.body.addEventListener('click', function(e) {
			...
			// enables <span data-debugger-href=""> & ctrl key
			if (e.ctrlKey && (link = closest(e.target, '[data-debugger-href]'))) {
				location.href = link.getAttribute('data-debugger-href');
				return false;
			}
			...
			// enables <a class="debugger-toggle" href="#"> or <span data-debugger-ref="#"> toggling
			if (link = closest(e.target, '.debugger-toggle')) { // eslint-disable-line
				var collapsed = link.classList.contains('debugger-collapsed'),
					ref = link.getAttribute('data-debugger-ref') || link.getAttribute('href', 2),
					dest = link;
				...
				link.classList.toggle('debugger-collapsed', !collapsed);
				dest.classList.toggle('debugger-collapsed', !collapsed);
				e.preventDefault();
			}
		});
	};
	...
	var build = function(data, repository, collapsed, parentIds) {
		...
		if (type === 'null' || type === 'string' || type === 'number' || type === 'boolean') {
			...
			return createEl(null, null, [
				createEl(
					'span',
					{'class': 'debugger-dump-' + type.replace('ean', '')},
					[data + '\n']
				)
			]);
			...
			return buildStruct(
				[
					createEl('span', {'class': 'debugger-dump-array'}, ['array']),
					' (' + (data[0] && data.length || '') + ')'
				],
				' [ ... ]',
			...
			return createEl(null, null, [
				createEl('span', {'class': 'debugger-dump-number'}, [data.number + '\n'])
			]);
			...
			return buildStruct(
				[
					createEl('span', {
						'class': data.object ? 'debugger-dump-object' : 'debugger-dump-resource',
						title: object.editor ? 'Declared in file ' + object.editor.file + ' on line ' + object.editor.line : null,
						'data-debugger-href': object.editor ? object.editor.url : null
					}, [object.name]),
					' ',
					createEl('span', {'class': 'debugger-dump-hash'}, ['#' + id])
				],
				' { ... }',
				object.items,
	...
	var buildStruct = function(span, ellipsis, items, collapsed, repository, parentIds) {
		...
		res = createEl(null, null, [
			toggle = createEl('span', {'class': collapsed ? 'debugger-toggle debugger-collapsed' : 'debugger-toggle'}, span),
			'\n',
			div = createEl('div', {'class': collapsed ? 'debugger-collapsed' : ''})
		]);
	...
	var createItems = function(el, items, repository, parentIds) {
		for (var i = 0; i < items.length; i++) {
			var vis = items[i][2];
			createEl(el, null, [
				createEl('span', {'class': 'debugger-dump-key'}, [items[i][0]]),
				vis ? ' ' : null,
				vis ? createEl('span', {'class': 'debugger-dump-visibility'}, [vis === 1 ? 'protected' : 'private']) : null,
				' => ',
