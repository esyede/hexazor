(function(){
	...
	var Panel = Debugger.DebugPanel = function(id) {
		this.id = 'debugger-debug-panel-' + id;
		this.elem = document.getElementById(this.id);
		this.elem.Debugger = this.elem.Debugger || {};
	};

	Panel.PEEK = 'debugger-mode-peek';
	Panel.FLOAT = 'debugger-mode-float';
	Panel.WINDOW = 'debugger-mode-window';
	Panel.FOCUSED = 'debugger-focused';
	Panel.zIndex = 20000;

	Panel.prototype.init = function() {
		var _this = this, elem = this.elem;

		elem.Debugger.onMove = function(coords) {
		...
		[].forEach.call(elem.querySelectorAll('.debugger-icons a'), function(a) {
	...
	Panel.prototype.focus = function(callback) {
		...
		if (this.is(Panel.WINDOW)) {
			elem.Debugger.window.focus();
		} else {
			clearTimeout(elem.Debugger.displayTimeout);
			elem.Debugger.displayTimeout = setTimeout(function() {
	...
	Panel.prototype.toWindow = function() {
		...
		doc.write('<!DOCTYPE html><meta charset="utf-8"><style>'
			+ document.getElementById('debugger-debug-style').innerHTML
			+ '<\/style><script>'
			+ document.getElementById('debugger-debug-script').innerHTML
			+ '<\/script><body id="debugger-debug">'
		);
		doc.body.innerHTML = '<div class="debugger-panel debugger-mode-window" id="' + this.id + '">' + this.elem.innerHTML + '<\/div>';
		win.Debugger.Dumper.init();
	...
	var Bar = Debugger.DebugBar = function() {
	};

	Bar.prototype.id = 'debugger-debug-bar';
	...
	Bar.prototype.init = function() {
		...
		[].forEach.call(elem.querySelectorAll('a'), function(a) {
			...
			a.addEventListener('mouseover', function(e) {
				if (isTargetChanged(e.relatedTarget, this) && this.rel && this.rel !== 'close' && !elem.classList.contains('debugger-dragged')) {
	...
	Bar.prototype.close = function() {
		document.getElementById('debugger-debug').style.display = 'none';
	};
	...
	var Debug = Debugger.Debug = {};

	Debug.init = function() {
		Debug.initResize();
		(new Bar).init();
		[].forEach.call(document.querySelectorAll('.debugger-panel'), function(panel) {
			Debug.getPanel(panel.id).init();
		});
	};

	Debug.getPanel = function(id) {
		return new Panel(id.replace('debugger-debug-panel-', ''));
	};
	...
	function setPosition(elem, coords) {
		if (elem.Debugger && elem.Debugger.onMove) {
			elem.Debugger.onMove.call(elem, coords);
		}
